...
class UpdateModelForDatasetId(BaseHandler):
    def get(self):
        ...
        if len(data)>0:
            ...
            model.save('/Users/yinzecui/Downloads/SwiftHTTPExample-InClassAssignment/turi_model_dsid%d' % (dsid))

        # send back the resubstitution accuracy
        # if training takes a while, we are blocking tornado!! No!!

        self.write_json({"resubAccuracy":acc})
        # print("Pause and wait 10 sec to see weather it's blocked")
        # time.sleep(10)
...
class PredictOneFromDatasetId(BaseHandler):
    def post(self):
        ...
        if(self.clf[dsid] == []):
            ...
            self.clf[dsid] = tc.load_model('/Users/yinzecui/Downloads/SwiftHTTPExample-InClassAssignment/turi_model_dsid%d' % (dsid))
